...
public class HigherTFCandles implements IIndicator, IDrawingIndicator {
    ...
    private TimeZone getTimezone() {
        switch (chartTimezone) {
            ...
            case 4: return TimeZone.getTimeZone("GMT+3:00");
            default: return TimeZone.getTimeZone("EET");
        }
    }
    ...
    private long getChartPeriodStart(long timeMillis, long intervalMs) {
        synchronized (chartCalendar) {
            chartCalendar.setTimeInMillis(timeMillis);
            if (intervalMs == 24 * 60 * 60 * 1000L) {
                chartCalendar.set(Calendar.HOUR_OF_DAY, 0); chartCalendar.set(Calendar.MINUTE, 0);
                chartCalendar.set(Calendar.SECOND, 0); chartCalendar.set(Calendar.MILLISECOND, 0);
            } else if (intervalMs == 7 * 60 * 60 * 1000L) {
                int h = chartCalendar.get(Calendar.HOUR_OF_DAY);
                chartCalendar.set(Calendar.HOUR_OF_DAY, (h / 7) * 7);
                chartCalendar.set(Calendar.MINUTE, 0); chartCalendar.set(Calendar.SECOND, 0);
                chartCalendar.set(Calendar.MILLISECOND, 0);
            } else {
                long dayStart = chartCalendar.getTimeInMillis();
                chartCalendar.set(Calendar.HOUR_OF_DAY, 0); chartCalendar.set(Calendar.MINUTE, 0);
                chartCalendar.set(Calendar.SECOND, 0); chartCalendar.set(Calendar.MILLISECOND, 0);
                dayStart = chartCalendar.getTimeInMillis();
                long diff = timeMillis - dayStart;
                long periods = diff / intervalMs;
                return dayStart + periods * intervalMs;
            }
            return chartCalendar.getTimeInMillis();
        }
    }
        ...
        for (int i = 0; i < MAX_LAYERS; i++) {
            ...
            if (showClosure && i == 0) {
                ...
                if (xOpenTime >= 0) {
                    ...
                    TimeZone chartTimeZone = support.getJFTimeZone().getTimeZone();
                    SimpleDateFormat labelFormat = new SimpleDateFormat("HH:mm");
                    labelFormat.setTimeZone(chartTimeZone);
                    long startTime = cd.openTime;
                    long endTime = startTime + periodMsClosure;
                ...
                if (cd.completed) {
                    long closeTime = cd.openTime + periodMsClosure;
                    ...
                    long futureCloseTime = cd.openTime + periodMsClosure;
