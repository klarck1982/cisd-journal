...
function buildTradeEvent(trade, timezone) {
  return {
    ...
    session: classifyTradeSession(trade, timezone),
...
function buildBacktestComparison(data, accountId) {
  ...
  return backtests.map((backtest) => {
    ...
    // Use the CISD formation time for the same chronology as the main analytics
    // event stream. importedAt is merely the time the CSV happened to be read.
    const events = reviewed.map((signal) => ({ at: signal.signalAt || signal.importedAt || '', result: toNumber(signal.resultR) }));
