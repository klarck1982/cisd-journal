...
function createStore(app) {
  ...
  const initial = () => ({
    version: 5,
    // New installations start empty. A funding firm becomes an account only
    // after the trader creates and connects it; demo firms must never look live.
    accounts: [],
    trades: [],
    openPositions: [],
    signals: [],
    ...
    importHistory: [],
    backtests: [],
    // Backtest transactions live in their own ledger and never enter real-account analytics.
    backtestTrades: [],
    playbooks: [],
  ...
  function normalize(data) {
    ...
    for (const key of ['accounts', 'trades', 'openPositions', 'signals', 'backtestSignals', 'backtests', 'backtestTrades', 'playbooks', 'daily', 'importHistory']) {
