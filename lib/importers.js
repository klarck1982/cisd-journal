...
const { calculateRMultiple } = require('./r-multiple');
...
function importError(code, fallback) {
...
function fundedNext(data, text, accountId, file) {
  ...
  if (rows.length < 2) throw importError('importEmptyCsv', 'The CSV file is empty.');
  ...
  if (required.some((key) => indexOf(key) < 0)) {
    throw importError('importNotFundedNext', 'This is not a FundedNext CSV in the expected format.');
  }
  ...
  rows.forEach((row, rowIndex) => {
    ...
    const closePrice = number(row[indexOf('Close Price')]);
    const derivedR = calculateRMultiple({ ...base, close: closePrice });
    data.trades.unshift({
      ...base,
      ...
      close: closePrice,
      resultR: derivedR,
      resultRSource: derivedR === null ? null : 'derived',
...
function mt5(data, text, accountId, file, isHtml) {
  ...
  if (headerAt < 0) throw importError('importNoMt5Table', 'No MT5 trade table was found in this report.');
  ...
  if (symbol < 0 || type < 0 || profit < 0) throw importError('importMt5ColumnsMissing', 'Required MT5 columns are missing from this report.');
  ...
  bodyRows.forEach((row, rowIndex) => {
    ...
    const slValue = sl >= 0 ? number(row[sl]) : 0;
    const derivedMt5R = calculateRMultiple({
    ...
    data.trades.unshift({
      ...
      resultR: derivedMt5R,
      resultRSource: derivedMt5R === null ? null : 'derived',
