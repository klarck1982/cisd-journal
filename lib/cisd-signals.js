...
// JForex writes ISO text, but Excel/older indicator exports may expose the
// same New York wall clock as M/D/YYYY H:mm. Parse it explicitly instead of
// relying on Date's machine-dependent string parser.
const SLASH_WALL_CLOCK_RE = /^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:[T ](\d{1,2}):(\d{2})(?::(\d{1,2}))?)?$/;
...
function parseNyWallClock(value, timeZone = NY_ZONE) {
  ...
  const slashMatch = text.match(SLASH_WALL_CLOCK_RE);
  if (slashMatch) {
...
function normalizeTimeframe(value) {
  ...
  if (!text) return '';
  if (text === 'h' || text === 'hour' || text === 'hours') return '60m';
  if (text === 'd' || text === 'day' || text === 'days') return '1440m';
  if (text === 'w' || text === 'week' || text === 'weeks') return '10080m';
  if (/^m\d+(?:\.\d+)?$/.test(text)) return `${Number(text.slice(1))}m`;
  if (/^\d+(?:\.\d+)?$/.test(text)) return `${Number(text)}m`;

  const match = text.match(/^(\d+(?:\.\d+)?)(m|min|mins|minute|minutes|h|hr|hrs|hour|hours|d|day|days|w|week|weeks)$/);
  if (!match) return text;
  const amount = Number(match[1]);
  const unit = match[2];
  if (['h', 'hr', 'hrs', 'hour', 'hours'].includes(unit)) return `${amount * 60}m`;
  if (['d', 'day', 'days'].includes(unit)) return `${amount * 1440}m`;
  if (['w', 'week', 'weeks'].includes(unit)) return `${amount * 10080}m`;
  return `${amount}m`;
}

function normalizeIndicatorSignal(signal = {}) {
  const normalized = { ...signal };
  const direction = normalizeCell(normalized.Direction);
  const signalId = normalizeCell(normalized.SignalID);
  const directionFromId = signalId.match(/_(BUY|SELL)(?:_|$)/i);

  // Excel displays a raw "+Cisd" value as #NAME? because it treats the plus
  // sign as a formula. The indicator's SignalID retains the authoritative BUY /
  // SELL token, so recover the display value without changing clean CSV rows.
  if (/^#NAME\??$/i.test(direction) && directionFromId) {
    normalized.Direction = directionFromId[1].toUpperCase() === 'BUY' ? '+Cisd' : '-Cisd';
  }
...
function matchesBacktestFilters(signal, filters = {}) {
  ...
  const timeframeFilters = arrayify(filters.timeframes || filters.tf).map(normalizeTimeframe);
  if (timeframeFilters.length && !timeframeFilters.includes(normalizeTimeframe(signal.TF))) return false;
...
function syncSignals(data, csvText, options = {}) {
  ...
  for (const rawSignal of parsed.rows) {
    diagnostics.scannedRows++;
    const signal = normalizeIndicatorSignal(rawSignal);
    if (!signal.SignalID) {
